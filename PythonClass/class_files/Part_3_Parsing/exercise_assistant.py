...
def review_numbers_total():
	info = {'name': "What's the total of the numbers in stanford.txt?"}
	analysis = ''
	a = b = 421 * 2
	results = ''
	...
	if not analysis:
		...
		if not analysis:
			if total == 0:
				...
				analysis = bad_answer_template.format("{} is a bit low...did you remove '$' characters?".format(total))
			elif total == 188:
				analysis = bad_answer_template.format("{} is still a bit low...did you remove '¢' characters?".format(total))
			elif total == 193:
				analysis = bad_answer_template.format("{} is still a bit low...did you remove ',' characters?".format(total))
			elif total < int((a + b) * 2.5):
				analysis = bad_answer_template.format("Getting there, but {} is still too low...".format(total))		
			elif total > int((a + b) * 2.5):
